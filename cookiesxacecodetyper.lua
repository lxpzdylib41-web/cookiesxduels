...
--// ============================================================
--//              🍪 COOKIES HUB STARTUP SPLASH
--// ============================================================

do
    --// Evitar splash duplicado
    pcall(function()
        local oldSplash = game:GetService("CoreGui"):FindFirstChild("CookiesHubSplash")
        if oldSplash then
            oldSplash:Destroy()
        end
    end)
    ...
    pcall(function()
        local oldSplash = playerGui:FindFirstChild("CookiesHubSplash")
        if oldSplash then
            oldSplash:Destroy()
        end
    end)
    ...
    splashGui.Name = "CookiesHubSplash"
    ...
    tapHint.Size = UDim2.new(1, 0, 0, 22)
    tapHint.Position = UDim2.new(0, 0, 1, -42)
    tapHint.BackgroundTransparency = 1
    tapHint.Text = "tap anywhere to skip"
    tapHint.TextColor3 = Color3.fromRGB(170, 100, 255)
    tapHint.Font = Enum.Font.Gotham
    tapHint.TextSize = 11
    tapHint.TextTransparency = 0.15
    tapHint.ZIndex = 10
    tapHint.TextXAlignment = Enum.TextXAlignment.Center
    tapHint.Parent = splashGui

    --// Zona para saltar
    local skipZone = Instance.new("TextButton")
    skipZone.Name = "SkipZone"
    skipZone.Size = UDim2.new(1, 0, 1, 0)
    skipZone.Position = UDim2.new(0, 0, 0, 0)
    skipZone.BackgroundTransparency = 1
    skipZone.BorderSizePixel = 0
    skipZone.Text = ""
    skipZone.AutoButtonColor = false
    skipZone.ZIndex = 9
    skipZone.Parent = splashGui

    --// Contenedor central
    local container = Instance.new("Frame")
    container.Name = "Container"
    container.Size = UDim2.new(0, 420, 0, 150)
    container.Position = UDim2.new(0.5, -210, 0, -180)
    ...
    titleSplash.Parent = container

    local titleGradient = Instance.new("UIGradient")
    titleGradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 255, 255)),
        ColorSequenceKeypoint.new(0.35, Color3.fromRGB(220, 150, 255)),
        ColorSequenceKeypoint.new(0.7, Color3.fromRGB(170, 70, 255)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(100, 20, 180))
    })
    titleGradient.Rotation = 0
    titleGradient.Parent = titleSplash

    --// Subtítulo
    local subSplash = Instance.new("TextLabel")
    subSplash.Name = "Subtitle"
    subSplash.Size = UDim2.new(1, 0, 0, 26)
    subSplash.Position = UDim2.new(0, 0, 0, 78)
    subSplash.BackgroundTransparency = 1
    subSplash.Text = "loading hub..."
    subSplash.TextColor3 = Color3.fromRGB(190, 120, 255)
    subSplash.Font = Enum.Font.GothamMedium
    subSplash.TextSize = 14
    subSplash.TextTransparency = 0
    subSplash.ZIndex = 3
    subSplash.Parent = container
    ...
    for i, txt in ipairs(fragTexts) do
        ...
        frag.Name = "Fragment" .. i
        frag.Size = UDim2.new(0, 130, 0, 65)
        frag.AnchorPoint = Vector2.new(0.5, 0.5)
        frag.Position = UDim2.new(
            0.5,
            (i - 2) * 105,
            0.5,
            -25
        ...
        frag.ZIndex = 5
        frag.Rotation = 0
        frag.Parent = splashGui

        table.insert(fragments, frag)
    end

    --// Sonidos
    local function playSound(id, pitch, volume, delayTime)
        task.delay(delayTime or 0, function()
            if not splashGui or not splashGui.Parent then
                return
            end
    ...
    local function playIntroSound()
        playSound("rbxassetid://1588058260", 0.85, 0.65, 0)
        playSound("rbxassetid://8627516764", 0.75, 0.45, 0.08)
        playSound("rbxassetid://1588058260", 1.15, 0.35, 0.16)
    end

    local function playImpact()
        playSound("rbxassetid://1588058260", 1.0, 0.9, 0)
        playSound("rbxassetid://8627516764", 0.9, 0.65, 0.04)
        playSound("rbxassetid://1588058260", 1.35, 0.45, 0.08)
    end
    ...
    local function doShatterEffect()
        ...
        flash.BackgroundColor3 = Color3.fromRGB(190, 90, 255)
        flash.BackgroundTransparency = 0.25
        flash.BorderSizePixel = 0
        flash.ZIndex = 8
        flash.Parent = splashGui
        ...
        for i, frag in ipairs(fragments) do
            ...
            local startX = frag.Position.X.Offset
            local startY = frag.Position.Y.Offset
    ...
    --// Saltar
    skipZone.Activated:Connect(function()
    ...
    while not splashDone and (tick() - startTime) < 6 do
        task.wait(0.05)
    end

    if not splashDone then
        finishSplash()
        ...
        if type(decoded) == "table" then
            ...
            if type(decoded.submitAfter) == "number" then
                savedConfig.submitAfter =
                    math.max(
                        1,
                        math.floor(decoded.submitAfter)
                    )
            end
...
local getupvalues =
    (debug and debug.getupvalues) or getupvalues

local getconns =
    getconnections or (debug and debug.getconnections)

local setupv =
    (debug and debug.setupvalue) or setupvalue
...
local RIDDLE_URL =
    "https://sab-riddle-solver.xyrcheatz.workers.dev"

--// Mantén aquí tu token si tu endpoint lo requiere.
local RIDDLE_TOKEN =
    "TU_TOKEN_AQUI"
...
local flashCode
local appendToBox
local rememberPendingSubmission
local clearPendingSubmission
local handleRedemptionFeedback
...
local function solveRiddle(message, seq)
    ...
    task.spawn(function()
        ...
        local top =
            (
                data
                and data.riddle == true
                and type(data.answers) == "table"
                and data.answers[1]
            )
            and normalizeCode(data.answers[1])
            or nil

        if top
        and #top > 0
        and _riddleSolver
        and seq >= _lastTypedSeq then
...
local function isOurGui(instance)
    ...
    for _ = 1, 10 do
        ...
        if p.Name == "ACECodeSniperUI"
        or p.Name == "SourcesHubRedeemerGui" then
            return true
        end
    ...
    if typeof(getconns) == "function" then
        ...
        if ok and type(cs) == "table" then
            ...
            for _, c in ipairs(cs) do
                ...
                and typeof(setupv) == "function" then
...
local function typeAndSubmitCode(code)
    ...
    local pg =
        playerGui
        or player:FindFirstChildOfClass("PlayerGui")
...
local COLORS = {
    ...
    Border =
        Color3.fromRGB(82, 82, 89),

    White =
        Color3.fromRGB(245, 245, 245),

    Text =
        Color3.fromRGB(190, 190, 196),
...
pcall(function()
    ...
    for _, name in ipairs({
        ...
        local previous =
            game.CoreGui:FindFirstChild(name)
...
local function scrollConsoleToBottom()
    ...
    task.defer(function()
        ...
        local bottom =
            math.max(
                0,
                Console.AbsoluteCanvasSize.Y
                - Console.AbsoluteWindowSize.Y
            )

        Console.CanvasPosition =
            Vector2.new(
                0,
                bottom
            )
...
local function appendConsoleStatus(
    name,
    activated
)
    ...
    local stateColor =
        activated
        and CONSOLE_COLORS.Green
        or CONSOLE_COLORS.Red
...
BrandMark.Position =
    UDim2.fromOffset(17, 15)

BrandMark.BackgroundColor3 =
    COLORS.Window

BrandMark.BackgroundTransparency =
    1

BrandMark.BorderSizePixel =
    0

BrandMark.ClipsDescendants =
    true
...
local AutoWriteButton =
    Instance.new("TextButton")

AutoWriteButton.Name =
    "AutoWrite"

AutoWriteButton.Size =
    UDim2.fromOffset(47, 24)

AutoWriteButton.Position =
    UDim2.new(
        1,
        -64,
        0,
...
AutoWriteButton.BackgroundColor3 =
    COLORS.Accent

AutoWriteButton.BorderSizePixel =
    0

AutoWriteButton.AutoButtonColor =
    false

AutoWriteButton.Active =
    true

AutoWriteButton.Text =
    ""

AutoWriteButton.ZIndex =
    5

AutoWriteButton.Parent =
    Header
...
AutoWriteKnob.BackgroundColor3 =
    COLORS.Window

AutoWriteKnob.BorderSizePixel =
    0

AutoWriteKnob.ZIndex =
    6

AutoWriteKnob.Parent =
    AutoWriteButton
...
local function toggleAutoWrite()
    ...
    autoWriteEnabled =
        not autoWriteEnabled

    _enabled =
        autoWriteEnabled

    if not autoWriteEnabled
    and clearAceCapture then

        clearAceCapture()
    end

    savedConfig.codeSniper =
        autoWriteEnabled
...
AutoWriteButton.Activated:Connect(
    toggleAutoWrite
)

AutoWriteButton.MouseButton1Click:Connect(
    toggleAutoWrite
)
...
local function makeStateButton(
    parent,
    enabled,
    consoleName,
    onToggle
...
CounterShell.Size =
    UDim2.fromOffset(96, 31)

CounterShell.Position =
    UDim2.new(
        1,
        -105,
        0.5,
...
CounterShell.BackgroundColor3 =
    COLORS.Window

CounterShell.BackgroundTransparency =
    0.05

CounterShell.BorderSizePixel =
    0
...
Console.BackgroundColor3 =
    COLORS.Log

Console.BorderSizePixel =
    0

Console.ClipsDescendants =
    true

Console.Active =
    true

Console.ScrollingEnabled =
    true

Console.ScrollingDirection =
    Enum.ScrollingDirection.Y

Console.ElasticBehavior =
    Enum.ElasticBehavior.WhenScrollable

Console.VerticalScrollBarInset =
...
Console.AutomaticCanvasSize =
    Enum.AutomaticSize.None

Console.ScrollBarThickness =
    4

Console.ScrollBarImageColor3 =
    COLORS.Dim
...
ConsoleOutput.AutomaticSize =
    Enum.AutomaticSize.Y

ConsoleOutput.Position =
    UDim2.fromOffset(
        9,
        6
    )

ConsoleOutput.BackgroundTransparency =
    1

ConsoleOutput.RichText =
    true
...
ConsoleOutput.TextSize =
    14

ConsoleOutput.Font =
    Enum.Font.Code

ConsoleOutput.TextColor3 =
    COLORS.Dim

ConsoleOutput.TextXAlignment =
    Enum.TextXAlignment.Left

ConsoleOutput.TextYAlignment =
    Enum.TextYAlignment.Top

ConsoleOutput.TextWrapped =
    true

ConsoleOutput.ZIndex =
    4

ConsoleOutput.Parent =
    Console

local CONSOLE_BOTTOM_PADDING =
    30
...
updateConsoleCanvas =
    function()
        ...
        if not Console
        or not ConsoleOutput then
            return
        end

        local contentHeight =
            ConsoleOutput.Position.Y.Offset
...
ConsoleOutput:GetPropertyChangedSignal(
    "AbsoluteSize"
):Connect(
    updateConsoleCanvas
)
...
DiscordFooter.TextXAlignment =
    Enum.TextXAlignment.Center

DiscordFooter.BackgroundColor3 =
    COLORS.Window

DiscordFooter.BackgroundTransparency =
    1

DiscordFooter.TextStrokeColor3 =
    COLORS.Window

DiscordFooter.TextStrokeTransparency =
    0.45

DiscordFooter.ZIndex =
    3
...
do
    ...
    local function isOverHeaderControl(position)
        ...
        local btnPos =
            AutoWriteButton.AbsolutePosition

        local btnSize =
            AutoWriteButton.AbsoluteSize

        return position.X >=
            (btnPos.X - 10)

            and position.X <=
            (btnPos.X + btnSize.X + 10)

            and position.Y >=
            (btnPos.Y - 10)

            and position.Y <=
            (btnPos.Y + btnSize.Y + 10)
    end
    ...
    Header.InputBegan:Connect(
        function(input)
            ...
            if dragging
            or isOverHeaderControl(input.Position) then
                return
            end
    ...
    UserInputService.InputChanged:Connect(
        function(input)
            ...
            if not dragging
            or not activeDragInput then
                return
            end

            local isTrackedTouch =
                activeDragInput.UserInputType
                    == Enum.UserInputType.Touch
                and input
                    == activeDragInput

            local isTrackedMouse =
                activeDragInput.UserInputType
                    == Enum.UserInputType.MouseButton1
                and input.UserInputType
...
local function col3ToRich(col)
    ...
    if col == COLORS.Text then
        return CONSOLE_COLORS.Amber
    end

    if col == COLORS.White then
        return CONSOLE_COLORS.Cyan
    end

    if col == COLORS.Dim then
        return CONSOLE_COLORS.Dim
    end
...
function setStatus(msg, col)
    ...
    or not _enabled then
        return
    end

    if msg == _lastStatusMsg then
        return
    end
...
UserInputService.TextBoxFocused:Connect(
    function(box)
        ...
        if box:IsDescendantOf(GUI) then
            return
        end
...
UserInputService.TextBoxFocusReleased:Connect(
    function(box)
        ...
        if box ~= codeBox
        and box ~= _lastBox then
            return
        end

        if _retypeInvalid
        and rememberPendingSubmission
        and (
            box == codeBox
            or box == _lastBox
        ) then
...
clearPendingSubmission =
    function()
        ...
        _pendingRejectedText =
            nil

        _pendingRejectedBox =
            nil

        _pendingRejectedUntil =
            0
    end
...
local function restoreRejectedText(
    box,
    previousText
)
    ...
    if not repasteBox
    or not isVisibleChain(repasteBox) then
        return false
    end
...
handleRedemptionFeedback =
    function(
        ...
        if not _retypeInvalid
        or not _pendingRejectedText then
            return
        end
        ...
        if feedbackObject
        and feedbackObject:IsDescendantOf(GUI) then
            return
        end

        local lower =
            tostring(text or ""):lower()
...
if aceNotifyRemote then
    ...
    if getgenv then
        ...
        local previous =
            getgenv().ACECodeSniperNotifyConnection
    ...
    aceListenConnection =
        aceNotifyRemote.OnClientEvent:Connect(
            function(...)
